...
#!/bin/bash
if [ $# != 1 ]; then
    echo "Please use: setup_build.sh [dist]"
    exit 1
fi
...
sbuild-createchroot \
    --make-sbuild-tarball=/var/lib/sbuild/${dist}.tar.gz \
    ${dist} `mktemp -d` ${url}
...
if [ "$disttype" = "ubuntu" ]; then
    cat <<__END__ | schroot -c source:${dist}-amd64-sbuild -d / -- tee /etc/apt/sources.list
